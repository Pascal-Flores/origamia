...
- Décision : déclinaison et review de 203–204 autorisées à la demande explicite de l'utilisateur, à partir des variantes 88–90 déjà en `testing`.
...
- 203 conserve `rit-remplir-auto-evaluation`, non réutilisé exactement dans `main`, mais reçoit une séquence plus développée et cinq affirmations comme les variantes 88–90.
- 204 abandonne `rit-reparer-couverture-cahier`, déjà utilisé par l'exercice 169, et utilise `rit-fin-seance-verifier-table`, non utilisé ailleurs dans le corpus vérifié.
...
- 203 contient une séquence pseudo-code de cinq actions encadrée par `Début` / `Fin`, avec une rupture vers la forme bloc et une instruction `écrire` incomplète. Cinq affirmations sont proposées, comme dans 88–90.
- 204 contient une séquence de blocs avec une phrase en langage naturel et un bloc `POUSSER` incomplet. Cinq affirmations permettent de distinguer ces problèmes de défauts absents sur le début, la fin et la précision d'un autre bloc.
...
- Déclinaison : autorisée pour 203–204
